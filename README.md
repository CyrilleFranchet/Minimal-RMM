...
With the server running, open **<http://127.0.0.1:8080/ui/>** (or your tunnel URL + `/ui/`). Paste your `RMM_API_TOKEN` to connect. **Dark mode** is the default; use the **☀ / ☽** toggle in the header to switch (saved in `localStorage`). The UI uses **WebSocket** (`/api/v1/ws`) for live session and output updates (new agents appear automatically; no manual refresh). The shell queues commands with **Enter**, runs `/exec` wait with **Ctrl+Enter**, and supports **↑/↓ command history** plus **Tab completion** (session history + `cmd:` / `PS:` / `powershell:` / `pwsh:` prefixes). Completed agent downloads appear in the **Downloads from agent** panel. **Session history** lists archived transcripts from killed sessions. Sidebar **Deploy agent (PowerShell)** generates a ready-to-run **`client_rmm.ps1`** (or config/env snippet) — see `docs/web-agent-generator.md`. The token is kept in `sessionStorage` for the browser tab only.

**AI Assistant** (button **AI** in the header): opens a chat panel on the right. Set your **OpenAI API key** in the panel (stored in `sessionStorage` for this tab). Conversation history is **saved per session on the server** (`RMM_logs/history/{id}/ai_chat.json`) and restored when you select that session; use **Reset chat** to start over (see `docs/web-ai-chat-memory.md`). The server runs an agent loop that spawns **`mcp_rmm_server.py`** over stdio and calls its tools (`POST /api/v1/ai/chat`). **Skills** — Markdown files in `ai-skills/` on the server are injected into the system prompt (see `docs/web-ai-skills.md`). Optionally enable **Exegol MCP** in the panel to merge tools from a running [Exegol MCP](https://docs.exegol.com/mcp/getting-started) server (HTTP, default `http://127.0.0.1:8000/mcp`). Install `pip install -r requirements.txt` (includes `mcp`; Python 3.10+). Set `RMM_AI_USE_MCP=0` to call `rmm_tools` directly without MCP. Server env: `RMM_EXEGOL_MCP_URL`, `RMM_EXEGOL_MCP_TOKEN`, `RMM_AI_SKILLS_DIR`. The selected session in the sidebar is passed as context.
...
| `GET` | `/sessions/{id}/ai/chat` | — | Load persisted AI chat for session (live or archived) |
| `DELETE` | `/sessions/{id}/ai/chat` | — | Clear persisted AI chat |
| `POST` | `/ai/chat` | `{"openai_api_key":"sk-…","messages":[…],"model":"gpt-5.2","skill_ids":[…],…}` | OpenAI agent loop via MCP (RMM + optional Exegol); persists chat when `selected_session_id` is set |
...
**Shared transcript:** commands and agent output for a session are stored on the server as **events** (`operator`, `output`, `config_ack`, …). Web UI and `rmm_cli.py` both poll `/api/v1/sessions/{id}/events` and receive WebSocket broadcasts — reopening a session shows the same history from any operator client. Large live WebSocket event bodies stay bounded, and the web UI hydrates truncated live results from the full REST transcript (see `docs/web-live-full-results.md`).
