...
To rebuild context for a new session, do the following in order:
...
3. Read all files modified on the current branch compared to `main` with `git diff --name-only main`.
