...
- [x] CMD `/c` quoting — `Join-RmmWindowsProcessArguments` + `ProcessStartInfo.WorkingDirectory` (no `cd /d` in script; fixes nested quotes and cwd-with-spaces)
...
| ~~CMD nested quotes / cwd~~ | **Fixed:** avoid `cd /d "…"` inside `/c` script; set `ProcessStartInfo.WorkingDirectory` instead. `Join-RmmWindowsProcessArguments` for `/c` argv (CreateProcess rules). |
